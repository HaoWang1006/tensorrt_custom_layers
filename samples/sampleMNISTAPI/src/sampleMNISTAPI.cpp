...
REGISTER_TENSORRT_PLUGIN(TestReluPluginCreator);
                                      ...
                                      SampleUniquePtr<nvinfer1::IBuilderConfig> &config)
{
  ...
  std::cout << "SampleMNISTAPI::constructNetwork" << std::endl;
  ...
  // add by wanghao
  auto creator = getPluginRegistry()->getPluginCreator("TestRelu", "1.0");
  const PluginFieldCollection *pluginData = creator->getFieldNames();
  IPluginV2 *pluginObj = creator->createPlugin("relulayer", pluginData);
  ITensor *inputTensors_yolo[] = {scale_1->getOutput(0)}; // scale_1->getOutput(0) data
  auto relu_1 = network->addPluginV2(inputTensors_yolo, 1, *pluginObj);
  relu_1->getOutput(0)->setName(mParams.outputTensorNames[0].c_str());
  network->markOutput(*relu_1->getOutput(0));
  ...
  // serial model
  std::ifstream engineFile("myleakyrelu.engine");
  if (!engineFile)
  {
    IHostMemory *modelStream = mEngine->serialize();
    assert(modelStream != nullptr);

    std::ofstream p("myleakyrelu.engine", std::ios::binary);
    if (!p)
    {
...
bool SampleMNISTAPI::infer()
{
  ...
  std::ifstream file("myleakyrelu.engine", std::ios::binary);
  ...
  if (file.good())
  {
    ...
    std::shared_ptr<nvinfer1::ICudaEngine> mEngine2 =
        SampleUniquePtr<nvinfer1::ICudaEngine>(runtime->deserializeCudaEngine(trtModelStream, size));
    assert(mEngine2 != nullptr);

    auto context2 = SampleUniquePtr<nvinfer1::IExecutionContext>(mEngine2->createExecutionContext());
    assert(context2 != nullptr);
    delete[] trtModelStream;

    samplesCommon::BufferManager buffers2(mEngine2, mParams.batchSize);

    // Read the input data into the managed buffers
    assert(mParams.inputTensorNames.size() == 1);
    if (!processInput(buffers2))
    {
      return false;
    }

    // Memcpy from host input buffers to device input buffers
    buffers2.copyInputToDevice();

    std::cout << "###################### 1111" << std::endl;
    bool status = context2->execute(mParams.batchSize, buffers2.getDeviceBindings().data());
    if (!status)
    {
      return false;
    }
    ...
    buffers2.copyOutputToHost();

    // Verify results
    if (!verifyOutput(buffers2))
    {
      return false;
    }
  }
...
bool SampleMNISTAPI::processInput(const samplesCommon::BufferManager &buffers)
{
  ...
  std::string proto_path = "/home/wangh/dev/tensorrt/data/mnist/mnist_mean.binaryproto"; // added by wanghao 10.10.2020
  auto meanBlob = SampleUniquePtr<nvcaffeparser1::IBinaryProtoBlob>(
      parser->parseBinaryProto(proto_path.c_str())); //locateFile(mParams.mnistMeansProto, mParams.dataDirs)
  if (!meanBlob)
  {
    return false;
  }
  ...
  std::cout << "hostDataBuffer === " << hostDataBuffer << std::endl;
  for (int i = 0; i < mParams.inputH * mParams.inputW; i++)
  {
    hostDataBuffer[i] = float(fileData[i]) - meanData[i];
    std::cout << i << "=" << hostDataBuffer[i] << " ";
  }
  std::cout << std::endl;
  std::cout << "----------------------------" << std::endl;
  return true;
...
bool SampleMNISTAPI::verifyOutput(const samplesCommon::BufferManager &buffers)
{
  ...
  // float maxVal{0.0f};
  // int idx{0};
  std::cout << "mParams.outputSize === " << mParams.outputSize << std::endl;
